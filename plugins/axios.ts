    ...
    if (error.response) {
        ...
        switch (status) {
            ...
            case 404:
                ...
                break;
            case 422:
                message = serverMessage || 'ข้อมูลไม่ผ่านการตรวจสอบ (Validation Failed)';
                break;
            case 500:
                message = 'เกิดข้อผิดพลาดจากทางเซิร์ฟเวอร์ (Internal Server Error)';
                break;
            default:
                message = serverMessage || `เกิดข้อผิดพลาดที่ไม่รู้จัก (Code: ${status})`;
        }
...
export const ApiHelper = {
    downloadFile: async (url: string, params?: any, fileName: string = 'file'): Promise<void> => {
